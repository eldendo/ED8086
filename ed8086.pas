...
function getReg(r: octal): byte;
begin
    if r<4 then getReg := lo(RX[r])
            else getreg := hi(RX[r-4])
end;

procedure setReg(r: octal; b: byte);
begin
    if r<4  then RX[r] := hi(RX[r])*256+b
            else RX[r-4] := b*256+lo(RX[r-4]) 
end;

function getReg2(r: octal): word;
begin
    getreg2 := RX[r]
end;

procedure setReg2(r: octal;b: word);
begin
    RX[r] := b 
end;

function getRegX(r: octal): word;
begin
    if w then getregX := getReg2(r) else getRegX := getReg(r)
end;

procedure setRegX(r: octal;b: word);
begin
